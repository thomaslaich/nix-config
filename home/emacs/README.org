...
Let's also adjust the line height:
#+begin_src emacs-lisp
...
*** Evil-org
~evil-collection~ bindings for ~org-mode~ are not great. The following package improves
...
** Toc-org
[[][toc-org]] allows to generate "Table of Contents" sections in org document by simply using the tag
...
[[][org-timeblock]] is a package that allows to display org-agenda items (like calendar events and TODOs)
in ASCII graphics similar to a program like Outlook.

I was using [[][calfw]] before, but the package is quite old and not very well-maintained. Furthermore, ~calfw~
